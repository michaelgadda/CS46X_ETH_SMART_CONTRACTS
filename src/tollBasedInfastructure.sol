...
contract tollInfrastructure {
    ...
    struct individualUser {
        ...
        string userLastName;
        uint256 userType; //different users have to pay different amounts
        uint256 totalInfrastructureUsage; //would need a way of knowing how much they have used the infrastructure
        uint256 totalPayed;
        string infName;
    }
    ...
    event printinfArray(
        string name, address payable wallet, uint256 totalRevenue, uint256 withdrawableRevenue, uint256 totalVisits
    );
    ...
    event insufficientFunds(
        uint256 amountToWithdraw, uint256 withdrawableRevenue, string infName, address payable infWallet
    );
    ...
    function checkInfNameIsNotAlreadyInArray(string memory infName) private view returns (bool) {
        for (uint256 i = 0; i < infStructArray.length; i++) {
    ...
    function addInfStructure(
        address payable infWallet,
        string memory infName,
        uint256 member_type1_cost,
        uint256 member_type2_cost,
        uint256 member_type3_cost,
        uint256 member_type1_threshold,
        ...
        require(
            msg.sender == owner,
            "You must be the owner of the contract to add a new infrastructure, please contact the owner!"
        );
        require(
            checkInfNameIsNotAlreadyInArray(infName) == false,
            "This Name is already in use, please choose another name and try again."
        );
    ...
    function addUserToNewInf(individualUser memory currUser, string memory infName)
        private
        returns (individualUser memory)
    {
    ...
    function findUserInDataBase(address payable userAddress, string memory infName)
        private
        returns (individualUser memory)
    {
        for (uint256 i = 0; i < userStructArray.length; i++) {
            if (
                userAddress == userStructArray[i].userWallet
                    && keccak256(bytes(infName)) == keccak256(bytes(userStructArray[i].infName))
            ) {
    ...
    function updateCurrInfRevenue(uint256 payed, infrastructure memory currInf) private {
        for (uint256 i = 0; i < infStructArray.length; i++) {
    ...
    function getAllInfArrayItems() public {
        for (uint256 i = 0; i < infStructArray.length; i++) {
            emit printinfArray(
                infStructArray[i].infName,
                infStructArray[i].infWallet,
                infStructArray[i].totalRevenue,
                infStructArray[i].withdrawableRevenue,
                infStructArray[i].totalVisits
            );
        }
    }
    ...
    function getAllIndUsersArrayItems() public {
        for (uint256 i = 0; i < userStructArray.length; i++) {
            emit printIndUserArray(
                userStructArray[i].userFirstName,
                userStructArray[i].userLastName,
                userStructArray[i].userWallet,
                userStructArray[i].totalPayed,
                userStructArray[i].userType
            );
        }
    }

    function getUserCost(individualUser memory currUser, infrastructure memory currInf)
        private
        pure
        returns (uint256 cost)
    {
        require(
            keccak256(bytes(currUser.infName)) == keccak256(bytes(currInf.infName)),
            "The user's infrastructure does not match the name of the infrastructure you are using.."
        );
    ...
    function updateUserTotalPayed(individualUser memory currUser, uint256 amountPayed) private {
        for (uint256 i = 0; i < userStructArray.length; i++) {
            if (currUser.userWallet == userStructArray[i].userWallet) {
    ...
    function updateInfWithdrawableAmount(infrastructure memory currInf, uint256 amountWithdrawed) private {
        for (uint256 i = 0; i < infStructArray.length; i++) {
            if (currInf.infWallet == infStructArray[i].infWallet) {
    ...
    function withdrawRevenue(uint256 amountToWithdraw) public {
        ...
        if (amountToWithdraw > infStruct.withdrawableRevenue) {
            emit insufficientFunds(
                amountToWithdraw, infStruct.withdrawableRevenue, infStruct.infName, infStruct.infWallet
            );
            ...
            emit successfulWithdrawal(
                infStruct.infName,
                infStruct.infWallet,
                "The funds were successfully withdrawn into the infrastuctures' wallet!"
            );
        }
    }
    ...
    function checkHowMuchIOwe(string memory infName) public {
        ...
        require(
            keccak256(bytes(currInf.infName)) != keccak256(bytes("NULL")),
            "We could not find the current infrastructure.."
        );
        uint256 userCost = getUserCost(currUser, currInf);
