...
contract Lending {
    ...
    event LoanRepaidInFull(
        address payable lender, address payable lendee, uint256 interestPayed, uint256 principleLoanPayed
    );
    ...
    function getLenderArrayIndex(address payable lenderAddress) private returns (uint256) {
    ...
    function getLendeeArrayIndex(address payable lendeeAddress) private returns (uint256) {
        for (uint256 i = 0; i < lendees.length; i++) {
    ...
    function checkifLoanExistsInArray(uint256 loanId) private returns (bool) {
    ...
    function checkIfLendeeHasAccessToLoan(address payable lendeeAddress, uint256 loanId) private returns (bool) {
        uint256 lendeeIndex = getLendeeArrayIndex(lendeeAddress);
        bool returnedTrue = false;
        if (lendeeIndex == 999999999) return false;
        //require(lendeeIndex != 999999999, "This lendee does not exist!");
        for (uint256 i = 0; i < lendees[lendeeIndex].loanIds.length; i++) {
    ...
    function checkifLenderHasAccessToLoan(address payable lenderAddress, uint256 loanId) private returns (bool) {
        uint256 lenderIndex = getLenderArrayIndex(lenderAddress);
        bool returnedTrue = false;
        //require(lenderIndex != 999999999, "This lendee does not exist!");
        if (lenderIndex == 999999999) return false;
        for (uint256 i = 0; i < lenders[lenderIndex].loanIds.length; i++) {
    ...
    function createLoan(
        address payable _lendee,
        address payable _lender,
        uint256 _loanAmount,
        uint256 _interestRate,
        uint256 _loanPeriod,
        uint256 _loanInstallmentPeriod
    ) public {
        ...
        loans.push(newLoan);
        lenderDeposit = true;
        uint256 lenderArrayIndex = getLenderArrayIndex(_lender);
        ...
        uint256 lendeeArrayIndex = getLendeeArrayIndex(_lendee);
        ...
        emit LoanCreated(
            "Please write down your loanId so you can access it at a later date.",
            newLoan.loanId,
            newLoan.lender,
            newLoan.lendee,
            newLoan.loanAmount,
            ...
            newLoan.loanPeriod,
            newLoan.loanInstallmentPeriod,
            newLoan.installmentAmount
        );
    }
    ...
    function repayInstallment(uint256 loanId) public payable {
        ...
        if (currTimeBwInstallments * 1 days > loans[loanId].daysBetweenInstallments) {
            ...
            currTimeBwInstallments * 1 days - loans[loanId].daysBetweenInstallments
                > loans[loanId].daysBetweenInstallments || block.timestamp > loans[loanId].loanEnd
        ) {
        ...
        if (loans[loanId].loanAmountLeft == 0 || loans[loanId].interestLeft == 0) {
            ...
            emit LoanRepaidInFull(
                loans[loanId].lender,
                loans[loanId].lendee,
                loans[loanId].interestPayed,
                loans[loanId].principleLoanPayed
            );
        }
        emit InstallmentRepaid(
            loans[loanId].lender,
            loans[loanId].lendee,
            loans[loanId].loanAmountLeft,
            loans[loanId].totalReceivedAmount,
            loans[loanId].principleLoanPayed,
            loans[loanId].loanRepaid
        );
    }
    ...
    function remainingTimeForCurrentInstallment(uint256 loanId) private {
        bool lendeeAccess = checkIfLendeeHasAccessToLoan(payable(msg.sender), loanId);
        bool lenderAccess = checkifLenderHasAccessToLoan(payable(msg.sender), loanId);
        require(lendeeAccess == true || lenderAccess == true, "You do not have access to this loan.");
        emit amountLeft(
            loanId,
            (
                loans[loanId].daysBetweenInstallments
                    - ((block.timestamp - loans[loanId].previousLoanInstallmentDate) * 1 days)
            )
        );
    ...
    function getMyActiveLoans() public {
        uint256[] memory fakeLoans;
        uint256 posLenderIndex = getLenderArrayIndex(payable(msg.sender));
        uint256 posLendeeIndex = getLendeeArrayIndex(payable(msg.sender));
        require(
            posLenderIndex != 999999999 || posLendeeIndex != 999999999,
            "This address is neither connected to a lender or lendee in our database; make sure you are using the correct wallet."
        );
        if (posLendeeIndex == 999999999) {
            ...
            emit myActiveLoans(
                "You are both a lendee and lender, here are your loanIds (The first series of Ids is for you lendee persona, the second is for your lender persona). ",
                lendees[posLendeeIndex].loanIds,
                lenders[posLenderIndex].loanIds
            );
        }
    }
